...
unsigned char* readBMP(const char* filename)
{
    ...
    {
    	cout<<"Sorry, NO such file in the directory";
    	exit(1);
	}
...
unsigned char* readBMP2(const char* filename)
{
	ofstream oFile;
...
int main()
{
	...
	cout<<"Give name of an image file in .bmp format "<<endl;
	cin>>fileName1;
	readBMP(fileName1.c_str());
	
	
	cout<<endl<<"Give another name of an image file in .bmp format which you want to check "<<endl;
	cin>>fileName2;
	readBMP2(fileName2.c_str());
	...
	string blockForFile;
	ofstream blockFile;
	cout<<endl<<"Give a file name in .txt format where you want to see the Blocks "<<endl;
	cin>>blockForFile;
	blockFile.open(blockForFile.c_str());
	cout<<endl<<"Wait a moment please..."<<endl;
	int count=0; 
	string previous="-1";
	node *newNode;
	...
	while(!inputFile1.eof())
	{
		...
		blockFile<<"Block Number"<<" "<<newNode[0].blockNo<<endl<<"Block Data "<<newNode[0].data<<endl<<"Current Hash "<<newNode[0].currentHash<<endl<<"Previous Hash  "<<newNode[0].previHash<<endl<<endl<<endl;
...
//	bool risk=false;
	...
	//inputFile.open("iFile.txt");
		...
		while(!hash1.eof())
		{
			...
			{
				check=true;
				cout<<"Don't match on block "<<blck<<endl;
				break;
			}
			else
			{
				cout<<"Block "<<blck<<" "<<"OK"<<endl;
			}
			
		}
		if(check==false) cout<<endl<<"Matched"<<endl;
		else cout<<endl<<"Pictures don't match "<<endl;
