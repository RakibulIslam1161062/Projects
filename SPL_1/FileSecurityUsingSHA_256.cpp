...
int main()
{
	...
	outHash.open("Hash.txt");
	ifstream inputFile;
	string filename;
	cout<<"Give an input File name  in .txt format which contains data"<<endl;
	cin>>filename;
	inputFile.open(filename.c_str());
	cout<<"Give a File name to store Blocks "<<endl<<endl;
	string out;
	cin>>out;
	...
	while(!inputFile.eof())
	{
		...
		cout<<"Block Number: "<<newNode[0].blockNo<<endl<<"Block Data: "<<newNode[0].data<<endl<<"Current Hash: "<<newNode[0].currentHash<<endl<<"Previous Hash: "<<newNode[0].previHash<<endl;
	...
	if(check=="YES") 
	{
		...
		while(!inputFile.eof())
		{
			...
			{
				cout<<"Data on Risk in Block "<<c<<endl;//blockNO<<endl;
				risk=true;
				break;
			}
			else
			{
				cout<<"Block "<<c<<" "<<"OK"<<endl;
			}
		
		}
}
	if(risk==false) cout<<"NO Danger!"<<endl;
